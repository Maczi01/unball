...
* **On submit, compute score** using linear deductions from caps:
    ...
    * *Initial $K_{km}$ and $K_y$: $K_{km} = 5$, $K_y = 400$ (to be finalized post-playtest).*
...
| **US-004** | Select a year for the event | As a player, I want to select a year so my temporal guess is recorded accurately. | **AC:** Year picker bounds are **1880 and 2025** inclusive. Non-numeric/out-of-range input is rejected. On submission, compute $|year\_guess - year\_true|$. |
