...
export function GameView({ mode, initialData, isAlreadySubmitted }: GameViewProps) {
	const [gameState, dispatch] = useReducer(
		gameReducer,
		{ mode, initialData, isAlreadySubmitted },
		(initial) => initializeGameState(initial.mode, initial.initialData, initial.isAlreadySubmitted),
	);
	...
	const handleSubmitWithNickname = useCallback(
		async (nickname: string, consent: boolean) => {
			...
			const guesses: GuessDTO[] = gameState.photos
				.filter((p) => p.guess !== null)
				.map((p) => p.guess as GuessDTO);
			...
			<div className="flex-1 p-4 md:p-6">
				<div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6">
					...
					<div className="space-y-6">
						...
						{/* Submit button */}
						<SubmitButton isDisabled={!isComplete || showFeedback} isLoading={gameState.isLoading} onClick={handleSubmitGuess} />
					</div>
					...
					<div className="lg:sticky lg:top-6 h-[400px] lg:h-[600px]">
						<MapComponent
							userPin={pin}
							correctPin={currentPhoto.result ? { lat: currentPhoto.result.correct_lat, lon: currentPhoto.result.correct_lon } : null}
